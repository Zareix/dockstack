...
- `just sqlc` — regenerate the sqlc Go code from `internal/db/queries/*.sql`
- `just openapi` — regenerate the checked-in OpenAPI spec (`wiki/openapi.yaml`) consumed by Orval
...
- `bun run web:dev` / `web:build` — Vite dev (port 5173, proxies `/api` + WS to the Go server on
  :3000) / production build
- `bun run --filter web typecheck` — `tsc --noEmit` (only available from `web/` directly)
- `bun run web:api-gen` — regenerate the Orval API client (`web/src/lib/api/generated/`) from
  `wiki/openapi.yaml` (script is `api:generate` in `web/`)
- `bun run web:lint` / `web:format` — oxlint (with `--fix`) / oxfmt scoped to `web/`
...
Always run `just lint` (golangci-lint + oxlint) and `go test ./...` before considering a Go change
...
adapter) — handlers are plain functions with typed input/output structs, and Huma generates the
OpenAPI 3.1 spec from them at runtime (served publicly at `/api/openapi.json` / `/api/docs`).
`api.Mount(router, deps)` registers everything. Public endpoints:
...
**Database queries (sqlc)**: SQL lives in `internal/db/queries/*.sql`; `just sqlc` (sqlc, config in
`sqlc.yaml`) generates typed Go code into `internal/db/store/` (package `store`). Never hand-edit
files in `internal/db/store/` — they are marked `// Code generated by sqlc. DO NOT EDIT.` Change
the SQL in `internal/db/queries/` (and migrations if needed) and regenerate.

**API spec → client pipeline (Orval)**: Huma's spec is checked in as `wiki/openapi.yaml`
(regenerate with `just openapi`). The frontend client is generated by Orval from that spec
(`web/orval.config.ts`, run `bun run web:api`) into `web/src/lib/api/generated/`, using a custom
fetch mutator (`web/src/lib/api/orval-mutator.ts`) wired to TanStack Query. Never hand-edit
`web/src/lib/api/generated/` — change backend handlers/spec and regenerate both ends.
